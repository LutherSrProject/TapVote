...
//Test this endpoint with curl http://localhost:8000/getSurveyTotalVotersByQuestion?surveyId=1

function getSurveyTotalVotersByQuestion(){
    var apiOptions = {};
    
    //The name of this route:
    apiOptions.endpointName = "getSurveyTotalVotersByQuestion";
    ...
    apiOptions.conclusion = function(data, response) {
        logger.info("Incoming request for survey total voters by question for: " + data['surveyId']);
        data['surveyId'] = parseInt(data['surveyId']);
        database.getSurveyTotalVotersByQuestion(data, function(err, results) {
            if (err) {
                if (!err["httpStatus"])
                    err["httpStatus"] = 500;
                ...
                if (!err["friendlyName"])
                    err["friendlyName"] = "Error getting survey total voters by question";

                httpresponses.errorResponse(err, response);
                return;
            }
            else {
                logger.info("Returning survey total voters by question");
